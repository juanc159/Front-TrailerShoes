...
const storeCharge = useCrudCalendarTypeStore()
const { action, calendarTypes, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeCharge)
...
const paginationData = computed(() => {
  const firstIndex = calendarTypes.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = calendarTypes.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeCharge.clearFormulario()
  storeCharge.typeAction = screen
  if (registryId)
    storeCharge.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeCharge.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    <HeaderAlertView title="Listado" sub-title="Tipo de calendario" icon="mdi-format-list-bulleted" />
    <VDataTable :headers="headers" :items="calendarTypes" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
      ...
      <template #top>
        <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
          ...
          <div class="app-user-search-filter d-flex  align-center flex-wrap gap-4">
            ...
            <VTooltip text="Tipo de calendario" location="top">
