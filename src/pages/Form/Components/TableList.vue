...
const { action, forms, totalPage, lastPage, currentPage, totalData, loading } = storeToRefs(storeForm)
...
const paginationData = computed(() => {
  const firstIndex = forms.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = forms.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeForm.clearFormulario()
  storeForm.typeAction = screen
  if (registryId && screen == 'form')
    storeForm.fetchInfo(registryId)
  // if (registryId && screen == 'report')
  //   storeForm.fetchInfoReport(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeForm.fetchDelete(id)
      await fetchDataList()
    }
...
<template>
  <div>
    <HeaderAlertView title="Listado" sub-title="Formularios" icon="mdi-format-list-bulleted" />
    <VDataTable :headers="headers" :items="forms" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
      ...
      <template #top>
        <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
          ...
          <div class="app-user-search-filter d-flex  align-center flex-wrap gap-4">
            ...
            <VTooltip text="Formulario" location="top">
