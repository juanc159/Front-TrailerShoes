...
const { action, products, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeProduct)
// --- --- END Store --- --- ---
...
const paginationData = computed(() => {
  const firstIndex = products.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = products.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeProduct.clearFormulario()
  storeProduct.typeAction = screen
  if (registryId) storeProduct.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeProduct.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Productos" icon="mdi-format-list-bulleted" />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="products" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #top>
