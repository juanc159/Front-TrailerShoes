...
const { action, socialNetworks, totalPage, lastPage, currentPage, totalData, loading } = storeToRefs(storeSocialNetwork)
// ---- ---- Endstore ------
...
const fetchDataList = async () => {
  await storeSocialNetwork.fetchAll({
...
const paginationData = computed(() => {
  const firstIndex = socialNetworks.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = socialNetworks.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeSocialNetwork.clearFormulario()
  storeSocialNetwork.typeAction = screen
  if (registryId)
    storeSocialNetwork.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeSocialNetwork.fetchDelete(id)
      await fetchDataList()
    }
...
<template>
  <div>
    <HeaderAlertView title="Listado" sub-title="Redes sociales" icon="mdi-format-list-bulleted" />
    <VDataTable :headers="headers" :items="socialNetworks" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
      ...
      <template #top>
        <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
          ...
          <VTooltip text="Red social" location="top">
