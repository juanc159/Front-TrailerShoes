...
const { action, styles, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeStyle)
// --- --- END Store --- --- ---
...
const paginationData = computed(() => {
  const firstIndex = styles.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = styles.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeStyle.clearFormulario()
  storeStyle.typeAction = screen
  if (registryId) storeStyle.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeStyle.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Estilos" icon="mdi-format-list-bulleted" />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="styles" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #top>
