...
const {
  action, dataList, totalPage, lastPage, currentPage, totalData, loading, typeAction
...
const paginationData = computed(() => {
  const firstIndex = dataList.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = dataList.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  companiesStore.clearFormulario()
  companiesStore.typeAction = screen
  if( registryId ) companiesStore.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await companiesStore.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Información" sub-title="Empresa"
      icon="mdi-information-variant"
    />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="dataList" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #top>
          <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
            ...
            <div class="app-user-search-filter d-flex  align-center flex-wrap gap-4">
              ...
              <VTooltip text="Agregar" location="top" v-if="dataList.length == 0"> 
