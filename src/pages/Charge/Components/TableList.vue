...
const { action, charges, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeCharge)
// --- --- END Store --- --- ---
...
const paginationData = computed(() => {
  const firstIndex = charges.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = charges.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  if (registryId) storeCharge.fetchInfo(registryId)
}
...
<template>
  <div>
    ...
    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="charges" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #top>
