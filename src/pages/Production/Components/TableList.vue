...
const { action, productions, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeProduction)
// --- --- END Store --- --- ---
...
const fetchDataList = async () => {
  await storeProduction.fetchAll({
...
const paginationData = computed(() => {
  const firstIndex = productions.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = productions.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeProduction.clearFormulario()
  storeProduction.typeAction = screen
  if (registryId) storeProduction.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeProduction.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Producciones" icon="mdi-format-list-bulleted" />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="productions" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #top>
          <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
            <div class="me-3" production="width: 80px;">
        ...
        <template v-if="loading" #body>
          <tr>
            <td colspan="6">
              <div production="width: 100;" class="d-flex justify-content-center">
                <PreloadInterno />
              </div>
            </td>
          </tr>
