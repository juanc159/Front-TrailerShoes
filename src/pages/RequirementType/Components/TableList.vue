...
const {
  action, requirementTypes, totalPage, lastPage, currentPage, totalData, loading
...
const fetchDataList = async () => {
  await storeRequirementType.fetchAll({
...
const paginationData = computed(() => {
  const firstIndex = requirementTypes.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = requirementTypes.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeRequirementType.clearFormulario()
  storeRequirementType.typeAction = screen
  if( registryId ) storeRequirementType.fetchInfo(registryId)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeRequirementType.fetchDelete(id)
      await fetchDataList()
    }
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Tipo de solicitudes"
      icon="mdi-format-list-bulleted"
    />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="requirementTypes" :items-per-page="rowPerPage"
        @update:sort-by="handleSortBy">
        <template #top>
          <VContainer fluid class="d-flex flex-wrap py-4 gap-4">
            <div class="me-3" style="width: 80px;">
              <VSelect :items="[10, 20, 30, 50]" density="compact" variant="outlined"
                v-model="rowPerPage"
              />
            </div>
            <VSpacer />
          ...
          <VCardText class="pt-2">
            <VRow>
              ...
              <VCol lg="8" cols="9" class="d-flex justify-end">
                <VPagination size="small" v-model="currentPage"
                  :total-visible="5" :length="lastPage"
                />
              </VCol>
            </VRow>
          </VCardText>
