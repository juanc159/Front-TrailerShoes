...
const {
  action, roles, totalPage, lastPage, currentPage, totalData, loading
} = storeToRefs(storeRole)
// --- --- END Store --- --- ---
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  if( registryId )
    storeRole.fetchInfo(registryId)
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Roles"
      icon="mdi-format-list-bulleted"
    />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="roles" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
        <template #item.actions="{ item }">
          <VBtn icon size="small" color="error" variant="text" @click="deleteData(item.value)">
          ...
          <VBtn icon size="small" color="default" variant="text" @click="changeScreen('form', item.value)">
